...
// # --------------------	   FFA Stage 1	----------------------
    ...
    //SNs1 = []
    // all_Ps1 = []
    for(int p0 = P0_start; p0 <= P0_end; p0++)  // P0s = np.arange(P0_start,P0_end,1) for p0 in P0s
    {
        float  M_real = N/p0;

        int stepen = qFloor(static_cast<double>(std::log2f(M_real))) + 1;
        float added_profs = static_cast<float>(pow2(static_cast<uint64_t>(stepen))) - M_real;
        if(p0==0 || p0 ==1)
        {
